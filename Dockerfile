...
RUN git clone https://github.com/go-yaml/yaml $GOPATH/src/gopkg.in/yaml.v2 && \
    ...
    git clone -b Alex-0301 https://github.com/PharbersDeveloper/MailServer.git $GOPATH/src/github.com/PharbersDeveloper/MailServer

#ADD  src/   $GOPATH/src/

# 设置工程配置文件的环境变量
ENV MAIL_HOME $GOPATH/src/github.com/PharbersDeveloper/MailServer/resources
ENV GO111MODULE on

# 构建可执行文件
RUN cd $GOPATH/src/github.com/PharbersDeveloper/MailServer && \
    go build && go install
